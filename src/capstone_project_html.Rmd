...
# - Only activate ggmap, ggplot2 and magrittr
# - Full specifications for everything else
#
...
> This might take a while. [Click here](https://github.com/roscoelai/dasr2020capstone#data) for a summary list of the primary data sources.

We work towards 2 seperate datasets: `data_time` for temporal analysis, and `data_space` for spatial analysis. They will only be fully available after the tidy and transform process since there's quite a bit of aggregating and joining involved.

We start with 7 raw datasets. The full details of their providence are given below.

### [MOH Weekly Infectious Disease Bulletin](https://www.moh.gov.sg/resources-statistics/infectious-disease-statistics/2020/weekly-infectious-diseases-bulletin)

```{r import-moh-bulletin}
...
### [MSS Historical Daily Weather Records](http://www.weather.gov.sg/climate-historical-daily/)

```{r import-mss-19stations}
...
### [MOH Healthcare Institutions Directory](http://hcidirectory.sg/hcidirectory/)

```{r import-hci-clinics}
...
> This is the preferred method as it's the fastest, but requires the files to be locally available.

```{r grand-import-local, results='hide', message=FALSE}
...
> Use this if you do not have a local copy of the files.

```{r grand-import-repo, eval=FALSE}
# Use the backup copies on GitHub (remember to maintain links!)
raw_data <- list(
  "moh_bulletin" = import_moh_weekly(paste0(
  ...
  "planning_areas" = read_kmls(paste0(
    "https://raw.githubusercontent.com/roscoelai/dasr2020capstone/master/",
    "data/kmls/plan-bdy-dwelling-type-2017.kml"
  )),
  ...
  "dengue_polys" = read_kmls(c(
    ...
    paste0("https://raw.githubusercontent.com/roscoelai/dasr2020capstone/",
           "master/data/kmls/denguecase-", ., "-area.kml")),
  ...
  "aedes_polys" = read_kmls(c(
    ...
    paste0("https://raw.githubusercontent.com/roscoelai/dasr2020capstone/",
           "master/data/kmls/breedinghabitat-", ., "-area.kml")),
...
> Use this if you have too much time.

```{r grand-import-de-novo, eval=FALSE}
# De novo (this might take a while, and requires a Google Map API key)
raw_data <- list(
  "moh_bulletin" = import_moh_weekly(paste0(
  ...
  "dengue_polys" = read_kmls(c(
    ...
    paste0("https://geo.data.gov.sg/denguecase-", .,
           "-area/2020/07/17/kml/denguecase-", .,
           "-area.kml")),
  ...
  "aedes_polys" = read_kmls(c(
    c(
      ...
      paste0("https://geo.data.gov.sg/breedinghabitat-", .,
             "-area/2020/07/17/kml/breedinghabitat-", .,
             "-area.kml"),
    c(
      "southeast",
      "southwest"
      ...
      paste0("https://geo.data.gov.sg/breedinghabitat-", .,
             "-area/2020/07/23/kml/breedinghabitat-", .,
             "-area.kml")
  )),
...
### A quick and dirty overview of the data available.
...
raw_data$mss_19stations <- raw_data$mss_19stations %>% 
  dplyr::mutate(
