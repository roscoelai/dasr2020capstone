...
# - Only activate ggfortify, ggmap, ggplot2 (notice a pattern?) and magrittr
# - Full specifications for everything else, for now
#
...
The full glorious details of their providence are given below. Unfortunately, the source code has been arranged with functional considerations in mind, and may not be in chronological order.

```{r import-helper-functions}
# These functions are mandatory (for importing raw data)
...
### Load raw data into memory (the easy)

> If the data files are available locally, set `online_repo = F` for faster loading. Otherwise, the function will read .csv files directly from the URLs while other file formats will be downloaded to a temporary folder.

```{r grand-import-no-webscraping, results='hide', message=FALSE}
grand_import_no_webscraping <- function(online_repo = TRUE) {
...
raw_data <- grand_import_no_webscraping(online_repo = F)
```

### Import data _de novo_ (where available)

> Use this if you have too much time. The following code chunk is disabled, but is provided here to show how the data can be derived from the primary sources (where available). The non-essential functions have been dumped here. Using this code chunk would require a valid Google Maps Platform API key.
...
# There are 2 options:
# 1. Incorporate into webscraping process
# 2. Incorporate into import (there will be 2 duplicates)
